...
public class IceFruit : ThrowableItem
{
                ...
                foreach (var trapPosition in bounds.allPositionsWithin)
                {
                    if (trapTilemap.HasTile(trapPosition))
                    {
                        ...
                        if (distance <= effectRadius)
                        {
                            ...
                            //Fountain
                            if (collider.GetComponent<Fountain>() != null)
                            {
                                //Turn off water flow
                                collider.GetComponent<Fountain>().DespawnWaterStream(trapPosition);
                            }

                            //Cold water (Pit)
                            else if (collider.GetComponent<ColdWater>() != null)
                            {
                                //Freeze pit over
                                collider.GetComponent<ColdWater>().Freeze(trapPosition);
                            }
                        }
                    }
                }
